...
func New(ss ShowtimeSvc, tracer opentracing.Tracer, logger jaegerLog.Factory) *Handler {
	return &Handler{
		showtimevc: ss,
		tracer:     tracer,
		logger:     logger,
...
// ShowtimeHandler ...
func (h *Handler) ShowtimeHandler(w http.ResponseWriter, r *http.Request) {
	var (
		ctx      context.Context
	...
	span := h.tracer.StartSpan("ShowtimeHandler", ext.RPCServerOption(spanCtx))
	defer span.Finish()
	...
	if err == nil {
		...
		case http.MethodGet:
			...
			if _id == true {
				...
				h.logger.For(ctx).Info("Running Service", zap.String("service", "GetShowtime"), zap.String("movieID", movieID))
				result, err = h.showtimevc.GetShowtime(ctx, movieID)
			}
