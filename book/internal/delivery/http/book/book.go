...
type BookSvc interface {
	GetShowDetail(ctx context.Context, movieID string) (bookEntity.BookDetail, error)
}
...
func New(bs BookSvc, tracer opentracing.Tracer, logger jaegerLog.Factory) *Handler {
	return &Handler{
		booksvc: bs,
		tracer:  tracer,
		logger:  logger,
...
func (h *Handler) BookHandler(w http.ResponseWriter, r *http.Request) {
	...
	span := h.tracer.StartSpan("BookHandler", ext.RPCServerOption(spanCtx))
	defer span.Finish()
	...
	if err == nil {
		...
		case http.MethodGet:
			...
			if _id == true {
				...
				h.logger.For(ctx).Info("Running Service", zap.String("service", "GetShowDetail"), zap.String("movieID", movieID))
				result, err = h.booksvc.GetShowDetail(ctx, movieID)
			}
